...
Many APIs implement some form of pagination: they break up large
...
need to edit your `010-call.R` file to implement pagination.
...
watch for pagination-related endpoint parameters, such as `page`,
`pageSize`, `perPage` `limit`, `offset`, or `cursor`.

For more tips on finding pagination information, see [How can I get a
lot of data from an
API?](https://r4ds.github.io/bookclub-wapir/slides/httr2/httr2-pagination.html)
in [Web APIs with R](https://r4ds.github.io/bookclub-wapir/).

## Implementing pagination

The [req_perform_iterative() function from
{httr2}](https://httr2.r-lib.org/reference/req_perform_iterative.html)
helps to implement pagination. It uses the request and some helper
functions to create a new request to fetch the next page. This family of
functions is experimental, so be sure to check the latest documentation
in case the functions have changed.

To implement pagination in your package, you will need to edit the
`010-call.R` file generated by {beekeeper}. By default, the perform step
is handled by
...
if you only give it a `req` object, or
[`httr2::req_perform_iterative()`](https://httr2.r-lib.org/reference/req_perform_iterative.html)
if you supply an iteration helper function in the `next_req` parameter.
For example, if every endpoint of your API uses a `page` parameter to
...
By default,
[`nectar::req_perform_opinionated()`](https://nectar.api2r.org/reference/req_perform_opinionated.html)
only returns 2 responses (`max_reqs = 2`). Once you have verified that
your pagination strategy works, you will likely want to increase this
limit, usually to `Inf`.
[`nectar::req_perform_opinionated()`](https://nectar.api2r.org/reference/req_perform_opinionated.html)
also implements a basic
[`httr2::req_retry()`](https://httr2.r-lib.org/reference/req_retry.html)
to try each request up to 3 times, using the default
`httr2::retry_retry()` settings to decide if a failure is transient.

## More complicated pagination

If you would like to implement more complex pagination, or apply other
transformations to the `req` object such as
[`httr2::req_retry()`](https://httr2.r-lib.org/reference/req_retry.html)
or
[`httr2::req_throttle()`](https://httr2.r-lib.org/reference/req_throttle.html),
you can create your own `perform` function. I name these functions
`{api_abbr}_req_perform()`. For example, this is the `perform` function
for the {fecapi} package:
